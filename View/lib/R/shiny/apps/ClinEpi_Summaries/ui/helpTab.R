help <-      tabItem(tabName = "help",
                 ...
                 #h5(
                   ...
                   p("For longitudinal datasets this app allows you to plot summary information for variables of interest against time. You will see at least one timeline and several drop-down menus at the top of the first tab of this application. The first set of drop-downs are labelled 'X-Axis' and 'Y-Axis', the second 'Facet Line' and the third 'Facet Plot'. The timeline(s) allow you to filter by time periods and age groups of interest. The X-axis menu appears when you have two timelines, and allows you to choose which variable between the two you would like to use as the X axis variable, as well as how many bins you would like to break this variable into. For example, the dataset you are using has both age and date variables which can be used longitudinally, then two timelines will appear and you will need to select whether to use dates or ages as the X axis. The Y-axis menu allows you to select a variable in the dataset which you would like to see summary data for. While the Facet Line menu allows you to select which variables you will see that summary data for and will draw lines on the resulting plot. For example, you may want to see the participants' average height for males and females over time. In this case, you would select height under the Y-axis menu and biological sex under the Facet Line menu. The Facet Plot menu is similar in concept to the Facet Line menu, except that rather than selecting a variable by which to draw lines you will select a variable for drawing seperate plots. Both the Facet Line and Facet Plot menus will allow you the options to plotall participants in a single group, to make your own groups based on a variable in the dataset, or to make groups based on all possible values for a variable."),
                   p("There are separate tabs where you will see tables and plots appear. If you update your selections in the menus, the tables and plots will update automatically."),
                   p("After making your selections a line graph will appear on the 'Plot Grid' and 'Individual Plots' tabs. If the Y-axis variable you selected is numeric, a series of vertical bars will also appear on the plot. These vertical bars function similarly to a box-and-whisker plot. The highest point of the line represents the maximum value for the Y-axis variable for that time point, and the lowest point the minimum value. The solitary points on these vertical bars similarly represent the 25th and 75th percentiles for each time point. Finally, a smoothed line with 95% confidence interval will be drawn for each group for the variable selected in Facet Line. If alternatively, you would like to see the actual calculated mean per time point, there is a radio button under the Y-axis menu which allows this. If, however, the Y-axis variable you selected is non-numeric then the radio button will allow you to choose between plotting the Y-axis as 'Count' or 'Proportion'. Please see 'Notes' for further details on plotting with 'Smoothed Conditional Mean'."),
                   ...
                   p("For non-longitudinal datasets this app allows you to plot summary information for variables of interest against other variables in the dataset. You will see several drop-down menus at the top of the first tab of this application. The first is labelled 'Y-axis', the second 'X-axis' and the third 'Facet Plot'. The Y-axis menu allows you to select a variable in the dataset which you would like to see summary data for. While the X-axis menu allows you to select which variable you will see that data plotted against. For example, you may want to see the participants' average height for males and females. In this case, you would select height under the Y-axis menu and biological sex under the X-axis menu. The Facet Plot menu will allow you to select a variable by which to draw seperate plots. The Facet Plot menu will allow you to either make your own groups based on a variable in the dataset, or to use all possible values for a variable."),
                   p("There are separate tabs where you will see the plots appear. If you update your selections in the menus, the plots will update automatically."),
                   p("After making your selections either a bar plot or a box-and-whisker plot will appear in the 'Plot Grid' and 'Individual Plots' tabs. If the Y-axis variable you selected is numeric, you will see a box-and-whisker plot. If, however, the Y-axis variable you selected is non-numeric then a bar plot will appear showing the proportion of participants for each X-axis group which meet the condition you selected for the Y-axis variable. If you would like to see the actual number of participants as opposed to the proportion, the radio button under the Y-axis menu will allow you to do this."),
